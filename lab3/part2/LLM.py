...
your_prompt = """在这个游戏中，墙壁和鬼魂是你无法移动到的位置，其余位置是可以到达的。你将操纵吃豆人进行移动来在尽可能少的步数内吃到所有的豆子，这要求你优先把附近的豆子全都吃掉，防止以后绕回来吃这些原本很近的豆子，而且尽量不要走到自己原来走过的地方，除非地图里只剩下不多的几个豆子，那么你就可以直接奔向这几个豆子吃掉即可（除了绕过墙壁和鬼魂）。注意，当吃豆人走到鬼魂的地方时便会死亡，鬼魂不会移动，因此你可以把鬼魂完全当成跟墙壁一样。
"""
