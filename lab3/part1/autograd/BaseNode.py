...
class Attention(Node):
    ...
    def cal(self, X):
        ...
        scores = scores - scores.max(axis=-1, keepdims=True)  # 防止exp溢出
        softmax_score = np.exp(scores)
        softmax_score /= np.sum(softmax_score, axis=-1, keepdims=True) + 1e-9
...
class Linear(Node):
    ...
    def backcal(self, grad):
        ...
        if X.ndim == 3:  # (B, L, D)
            self.grad.append(np.einsum("bld,ble->de", X, grad))
            self.grad.append(np.sum(grad, axis=(0, 1)))
            return np.einsum("ble,ed->bld", grad, self.params[0])
        else:
            if X.ndim == 1:
                X = X[None, :]
            if grad.ndim == 1:
...
class ResLinear(Node):
    ...
    def cal(self, X):
        ...
        # 即Linear(X)+X
        self.cache.append(X)
        return X @ self.params[0] + self.params[1] + X  # 加残差
    ...
    def backcal(self, grad):
        ...
        if X.ndim == 3:
            self.grad.append(np.einsum("bld,ble->de", X, grad))
            self.grad.append(np.sum(grad, axis=(0, 1)))
            return np.einsum("ble,ed->bld", grad, self.params[0]) + grad
        else:
            if X.ndim == 1:
                X = X[None, :]
            if grad.ndim == 1:
...
class LogSoftmax(Node):
    ...
    def cal(self, X):
        ...
        logsumexp = np.log(np.sum(np.exp(X), axis=self.dim, keepdims=True))
        ret = X - logsumexp
    ...
    def backcal(self, grad):
        ...
        return grad - softmax * np.sum(grad, axis=self.dim, keepdims=True)
