...
def buildTree(X: np.ndarray, Y: np.ndarray, unused: List[int], depth: int, purity_bound: float, gainfunc: Callable, prefixstr=""):
    ...
    # 当达到终止条件时，返回叶节点
    # TODO: YOUR CODE HERE
    if len(u) == 1 or len(unused) == 0 or depth == 0 or entropy(Y) < purity_bound:
        return root
    ...
    for u in ufeat:
        if np.sum(feat == u) == 0:
            continue
        root.children[u] = buildTree(X[feat == u], Y[feat == u], unused, depth-1, purity_bound, gainfunc, prefixstr + "  ")
    return root
