...
class CrossEntropyLoss(Node):
    ...
    def cal(self, X):
        ...
        EPS = 1e-6
        self.cache.append(X)
        return -np.sum(np.log(np.take_along_axis(X + EPS, np.expand_dims(self.y, axis=-1), axis=-1)))
    ...
    def backcal(self, grad):
        ...
        X = self.cache[-1]
        ret = np.zeros_like(X)
        np.put_along_axis(ret, np.expand_dims(self.y, axis=-1), -1 / (np.take_along_axis(X + EPS, np.expand_dims(self.y, axis=-1), axis=-1)), axis=-1)
        return grad * ret