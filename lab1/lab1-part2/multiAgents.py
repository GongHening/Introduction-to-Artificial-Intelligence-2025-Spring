...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth, index_of_agent):
            ...
            "*** YOUR CODE HERE ***"
            legalMoves = state.getLegalActions(index_of_agent)
            for action in legalMoves:
                if minimizer(state.generateSuccessor(index_of_agent,action),depth,1)[0]>value:
                    value=minimizer(state.generateSuccessor(index_of_agent,action),depth,1)[0]
                    maxiAction=action
        ...
        def minimizer(state, depth, index_of_agent):
            ...
            for action in legalMoves:
                if index_of_agent==state.getNumAgents()-1:
                    if maximizer(state.generateSuccessor(index_of_agent,action),depth-1,0)[0]<value:
                        value=maximizer(state.generateSuccessor(index_of_agent,action),depth-1,0)[0]
                        miniAction=action
                else:
                    if minimizer(state.generateSuccessor(index_of_agent,action),depth,index_of_agent+1)[0]<value:
                        value=minimizer(state.generateSuccessor(index_of_agent,action),depth,index_of_agent+1)[0]
                        miniAction=action
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def maximizer(state, depth, index_of_agent, alpha, beta):
            ...
            legalMoves = state.getLegalActions(index_of_agent)
            for action in legalMoves:
                if minimizer(state.generateSuccessor(index_of_agent,action),depth,1,alpha,beta)[0]>value:
                    value=minimizer(state.generateSuccessor(index_of_agent,action),depth,1,alpha,beta)[0]
                    maxiAction=action
                if value>alpha:
                    alpha=value
                if alpha>beta:
        ...
        def minimizer(state, depth, index_of_agent, alpha, beta):
            ...
            for action in legalMoves:
                if index_of_agent==state.getNumAgents()-1:
                    if maximizer(state.generateSuccessor(index_of_agent,action),depth-1,0,alpha,beta)[0]<value:
                        value=maximizer(state.generateSuccessor(index_of_agent,action),depth-1,0,alpha,beta)[0]
                        miniAction=action
                    if value<beta:
                        beta=value
                    if alpha>beta:
                    ...
                    if minimizer(state.generateSuccessor(index_of_agent,action),depth,index_of_agent+1,alpha,beta)[0]<value:
                        value=minimizer(state.generateSuccessor(index_of_agent,action),depth,index_of_agent+1,alpha,beta)[0]
                        miniAction=action
                    if value<beta:
                        beta=value
                    if alpha>beta:
            ...
            for action in actions:
                ...
                if action=='Stop':
                    ...
                    if cgstree.stop==None:
                        flag=True
                        break
            if flag or children==[] or cgs.isWin() or cgs.isLose():
                return (cgs,cgstree)
